...
逐条对照 docs/user-manual/user-manual.md 中记录的预期数值，
...
def path_python(task, df, target, features, params=None, raw_cat=False):
    ...
    req = AnalysisRequest(
        task=task, data=df_enc, target_col=target, feature_cols=feat_enc, params=merged
    )
...
def path_cli(task, df, target, features, params=None):
    ...
    """
    return path_python(task, df, target, features, params, raw_cat=(task in RAW_CAT_TASKS))
...
def path_web(task, df, target, features, params=None, categoricals=None):
    ...
    if categoricals is None:
        categoricals = []
    results = run_analysis(task, df, [target], list(features), list(categoricals), params)
...
def test_manual_4_1_correlation(raw_df):
    ...
    for path_name, r in [("Python", r_py), ("CLI", r_cli), ("Web", r_web)]:
        target_corr = (
            r.metadata.get("target_correlations", {})
            if hasattr(r, "metadata")
            else r.get("metadata", {}).get("target_correlations", {})
        )
        if target_corr:
            top = max(target_corr.items(), key=lambda kv: abs(kv[1]))
            top_name, top_r_val = top
            # 手册预期: 最强 = 注射压力, |r| ≈ 0.050
            assert abs(top_r_val) < 0.1, f"{path_name}: |r|={abs(top_r_val):.4f} 应在 0.05 附近"
    ...
    web_top = (
        abs(list(r_web["metadata"].get("target_correlations", {}).values())[0])
        if "target_correlations" in r_web.get("metadata", {})
        else 0
    )
...
def test_manual_4_2_anova(raw_df):
    ...
    for path_name, r in [("Python", r_py), ("Web", r_web)]:
        ...
        assert 0.5 < p_val < 0.8, f"{path_name}: p={p_val:.4f}，手册预期≈0.615"
        assert 0.001 < eta2 < 0.01, f"{path_name}: η²={eta2:.4f}，手册预期≈0.0027"

    # 数值交叉验证
    py_p = float(r_py.metadata.get("p_value", 999))
    web_p = float(r_web.get("metadata", {}).get("p_value", 999))
    assert abs(py_p - web_p) < 1e-4, f"anova p: Python={py_p:.4f}, Web={web_p:.4f}"
...
def test_manual_4_3_hypothesis_test(raw_df):
    ...
    for path_name, r in [("Python", r_py), ("Web", r_web)]:
        ...
        assert p_val < 0.001, f"{path_name}: p={p_val:.6f}，手册预期<0.001"
        assert 1.0 < abs(d_val) < 1.6, f"{path_name}: d={d_val:.3f}，手册预期≈1.31"

    # 均值验证: 保养日=否 ≈4.41, 是≈2.89
    py_p = float(r_py.metadata.get("p_value", 999))
    web_p = float(r_web.get("metadata", {}).get("p_value", 999))
    assert abs(py_p - web_p) < 1e-6, f"hypothesis_test p: Python={py_p:.6f}, Web={web_p:.6f}"
...
def test_manual_4_4_decision_tree(raw_df):
    ...
    for path_name, r in [("Python", r_py), ("CLI", r_cli), ("Web", r_web)]:
        ...
        # 冷却时间应排第一
        assert "冷却时间" in top_name, f"{path_name}: 最重要因子应为冷却时间，实际: {top_name}"
...
def test_manual_4_5_vif(raw_df):
    ...
    for path_name, r in [("Python", r_py), ("Web", r_web)]:
        ...
        assert high_vif == 0, f"{path_name}: 高VIF数量={high_vif}，手册预期=0（全部正常）"
        ...
        assert max_vif < 2.0, f"{path_name}: 最大VIF={max_vif:.3f}，手册预期≈1.0"
...
def test_manual_4_7_proportion_ci(raw_df):
    ...
    for path_name, r in [("Python", r_py), ("Web", r_web)]:
        ...
        if ci_table is not None and hasattr(ci_table, "iloc"):
            point_est = float(ci_table.iloc[0, 0]) if len(ci_table) > 0 else 0
            assert 0.88 < point_est < 0.94, f"{path_name}: 点估计={point_est:.4f}，手册预期≈0.911"
...
def test_manual_5_1_cohens_kappa(raw_df):
    ...
    for path_name, r in [("Python", r_py), ("Web", r_web)]:
        ...
        assert abs(kappa) < 0.1, f"{path_name}: kappa={kappa:.4f}，手册预期≈-0.009（低于随机）"
...
def test_manual_5_2_cronbach_alpha(raw_df):
    ...
    r_py, _, r_web, py_st, _, web_st = _compare_3paths(
        "cronbach_alpha", raw_df, "", ["熔体温度", "模具温度", "注射压力"], {}, [], raw_cat=False
    )
    ...
    for path_name, r in [("Python", r_py), ("Web", r_web)]:
        ...
        assert alpha < 0.7, f"{path_name}: α={alpha:.3f}，手册预期<0.7"
        # α 可能为负（已修复：负值会附加警告消息）
        if alpha < 0:
            msgs = r.messages if hasattr(r, "messages") else r.get("messages", [])
            assert any("负值" in str(m) for m in msgs), f"{path_name}: 负 α 应有警告消息"
...
def test_manual_5_3_distribution_summary(raw_df):
    ...
    for path_name, r in [("Python", r_py), ("Web", r_web)]:
        ...
        assert "Normal" in str(best_fit), f"{path_name}: best_fit={best_fit}，手册预期=Normal"
...
def test_manual_5_4_normality_check(raw_df):
    ...
    for path_name, r in [("Python", r_py), ("Web", r_web)]:
        ...
        assert n_columns == 2, f"{path_name}: n_columns={n_columns}，预期=2"
...
def test_manual_5_5_power_analysis(raw_df):
    ...
    for path_name, r in [("Python", r_py), ("Web", r_web)]:
        ...
        assert 60 <= req_n <= 70, f"{path_name}: required_n={req_n}，手册预期=64"
...
def test_manual_6_1_regression(raw_df):
    ...
    for path_name, r in [("Python", r_py), ("CLI", r_cli), ("Web", r_web)]:
        ...
        # R² 应很小（测试数据随机生成）
        assert r_sq < 0.05, f"{path_name}: R²={r_sq:.4f}，手册预期很小（随机数据）"

        # 验证系数表存在
        tables = r.tables if hasattr(r, "tables") else r.get("tables", {})
        assert "coefficients" in tables, f"{path_name}: 缺少 coefficients 表"

    # 三路径 R² 一致
    py_r2 = float(r_py.metadata["r_squared"])
    web_r2 = float(r_web["metadata"].get("r_squared", 0))
    assert abs(py_r2 - web_r2) < 1e-4, f"regression R²: Python={py_r2:.4f}, Web={web_r2:.4f}"
...
def test_manual_spc_xbar(raw_df):
    ...
    r_py, _, r_web, py_st, _, web_st = _compare_3paths(
        "spc_xbar", spc_df, "不良率", ["_子组"], {}, [], raw_cat=False
    )
    ...
    for path_name, r in [("Python", r_py), ("Web", r_web)]:
        ...
        assert has_cl_info, f"{path_name}: 缺少控制限信息 (keys={list(meta.keys())[:6]})"
...
def test_all_methods_3path_behavior(raw_df, task):
    ...
    # 验证行为一致
    assert py_st == web_st, f"{task}: Python={py_st}, Web={web_st} — 行为不一致！"
