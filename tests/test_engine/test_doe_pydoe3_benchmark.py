"""DOE 设计结果 vs pyDOE3 交叉验证（benchmark，可选测试依赖）。

pyDOE3 是外部基准库（pip install pyDOE3）；未安装时本模块自动跳过。
...
编码约定：我方二水平用 0/1，pyDOE3 用 -1/+1，比较前统一转 0/1。

注：CCD 的 alpha='orthogonal' 不与 pyDOE3 比较——pyDOE3 的 'orthogonal'
...
def test_fullfact_matches_pydoe3():
