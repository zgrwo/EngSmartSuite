...
def _setup_engine_dir(tmp_path, monkeypatch, file_code):
    engine_dir = tmp_path / "engine"
    engine_dir.mkdir()
    sample = engine_dir / "sample.py"
    sample.write_text(file_code, encoding="utf-8")
    # audit_file 用 ROOT 做 relative_to 展示，main() 读模块级 ENGINE_DIR——一并替换保持封闭
    monkeypatch.setattr(falsy_audit, "ROOT", tmp_path)
    monkeypatch.setattr(falsy_audit, "ENGINE_DIR", engine_dir)
    return sample
...
def test_negative_injection_threshold_exits_1(tmp_path, monkeypatch):
    ...
    sample = _setup_engine_dir(tmp_path, monkeypatch, BAD_CODE)
...
def test_real_engine_dir_zero_high():
    """真实 engine/ 仓库态守卫：零 HIGH（若引入 HIGH 模式应修源码，而非放宽本测试）。"""
