...
r1 = orchestrate(AnalysisRequest(task='correlation', data=d1, target_col='y', feature_cols=['x1','x2']))
r2 = orchestrate(AnalysisRequest(task='correlation', data=d2, target_col='y', feature_cols=['x1','x2']))
...
r1r2 = orchestrate(AnalysisRequest(task='regression', data=d1, target_col='y', feature_cols=['x1','x2'])).metadata['r_squared']
r2r2 = orchestrate(AnalysisRequest(task='regression', data=d2, target_col='y', feature_cols=['x1','x2'])).metadata['r_squared']
check("Same input -> same R2", abs(r1r2 - r2r2) < 1e-10)

# ============================================================
section("4. Error Handling & Edge Cases")
# ============================================================
check("Unknown task -> error", orchestrate(AnalysisRequest(task='nonexistent', data=d1, target_col='y')).status == 'error')
check("Missing target col -> error", orchestrate(AnalysisRequest(task='anova', data=d1, target_col='no_such_col', feature_cols=['x1'])).status == 'error')
r = orchestrate(AnalysisRequest(task='correlation', data=d1.assign(x1_nan=d1['x1'].where(d1.index>5)), target_col='y', feature_cols=['x1','x2']))
...
r = orchestrate(AnalysisRequest(task='correlation', data=d1, target_col='y', feature_cols=['x1','x2']))
...
rs = orchestrate(AnalysisRequest(task='response_surface', data=d1, target_col='y', feature_cols=['x1','x2']))
...
if os.path.exists(test_path):
    ...
    r = orchestrate(AnalysisRequest(task='correlation', data=df, target_col='拉伸强度', feature_cols=nc))
    check("melt_temp correlates with tensile_strength", '熔体温度' in r.summary, r.summary)
    r = orchestrate(AnalysisRequest(task='anova', data=df.dropna(subset=['冲击强度']), target_col='冲击强度', feature_cols=['原料类型'], params={'alpha':0.05}))
    check("material_type significantly affects impact_strength", '显著' in r.summary, r.summary)
    r = orchestrate(AnalysisRequest(task='anova', data=df.dropna(subset=['不良率']), target_col='不良率', feature_cols=['保养日'], params={'alpha':0.05}))
    check("maintenance_day significantly affects defect_rate", '显著' in r.summary, r.summary)
    r = orchestrate(AnalysisRequest(task='anova', data=df.dropna(subset=['不良率','设备报警']), target_col='不良率', feature_cols=['设备报警'], params={'alpha':0.05}))
...
r = orchestrate(AnalysisRequest(task='correlation', data=pd.DataFrame({'x':x,'y':y}), target_col='y', feature_cols=['x']))
...
r = orchestrate(AnalysisRequest(task='hypothesis_test', data=pd.concat([g1,g2]), target_col='v', feature_cols=['g'], params={'group_col':'g'}))
