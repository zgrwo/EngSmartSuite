...
mold_ids = np.random.choice(
    ["MOLD-A01", "MOLD-A02", "MOLD-B01", "MOLD-B02", "MOLD-C01", "MOLD-C03"], N
)
...
material_types = np.random.choice(
    ["ABS", "PP", "PA6", "PC", "PA66"], N, p=[0.3, 0.25, 0.2, 0.15, 0.1]
)
material_batches = [
    f"BATCH-{random.choice(['A', 'B', 'C'])}{random.randint(100, 999)}" for _ in range(N)
]
...
ambient_conditions = np.random.choice(
    ["常温常湿", "高温高湿", "低温低湿", "常温高湿"], N, p=[0.5, 0.2, 0.15, 0.15]
)
product_codes = np.random.choice(
    ["P-101", "P-102", "P-201", "P-202", "P-301"], N, p=[0.3, 0.25, 0.2, 0.15, 0.1]
)
...
df = pd.DataFrame(
    {
        ...
        "检验员": inspectors,
        "原料类型": material_types,
        "原料批号": material_batches,
        "冷却方式": cooling_methods,
        "循环模式": cycle_modes,
        "模温分区": mold_temp_zones,
        "保养日": maintenance_days,
        "环境条件": ambient_conditions,
        "产品代码": product_codes,
        "色母": color_masterbatch,
        "嵌件类型": insert_types,
        "首件合格": first_article_ok,
        "外观检查": visual_inspection,
        "尺寸检查": dimensional_check,
        "需返工": rework_required,
        "设备报警": machine_alarm,
        "换料": material_change,
        "熔体温度": melt_temp_base.round(1),
        "模具温度": mold_temp.round(1),
        "注射压力": injection_pressure.round(1),
        "保压压力": holding_pressure.round(1),
        "注射速度": injection_speed.round(1),
        "冷却时间": cooling_time.round(1),
        "循环周期": cycle_time.round(1),
        "螺杆转速": screw_speed.round(0),
        "背压": back_pressure.round(1),
        "锁模力": clamping_force.round(0),
        "干燥温度": drying_temp.round(1),
        "干燥时间": drying_time.round(1),
        "拉伸强度": tensile_strength.round(2),
        "断裂伸长率": elongation.round(2),
        "冲击强度": impact_strength.round(2),
        "表面粗糙度": surface_roughness.round(3),
        "不良率": defect_rate.round(3),
        "尺寸偏差": dimensional_deviation.round(3),
        "重量波动": weight_variation.round(3),
        "循环效率": cycle_efficiency.round(1),
    }
)
