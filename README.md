...
- Windows（macOS 也可运行 CLI/Python 模式，但 Excel 功能区集成仅 Windows 完整支持）
- Python 3.10 或更高版本
- Microsoft Excel
...
### 第二步：配置 Excel 功能区（三种方式，任选其一）

#### 方式 A：直接打开加载项工作簿（最简单，推荐先用这个验证）

项目自带预配置的 `smartsuite_addin.xlsm`，双击打开即可使用：

```
smartsuite_addin/smartsuite_addin.xlsm
```

打开后 Excel 会提示"是否启用宏"，选择 **"启用内容"**。这个文件已集成 xlwings Python 调用能力——虽然没有自定义功能区，但你可以在 VBA 即时窗口（Alt+F11 → Ctrl+G）中输入：
...
#### 方式 B：安装为正式加载项（带"工艺分析"功能区标签）

如果你想在 Excel 任何工作簿中看到 **"工艺分析"** 功能区标签：
...
2. 菜单：**文件 → 导入文件** → 选择 `smartsuite_addin/SmartSuiteRibbon.bas`
3. 用 [Custom UI Editor](https://github.com/fernandreu/office-ribbonx-editor) （免费工具）打开 `smartsuite_addin/SmartSuite_Addin.xlam`
4. 将 `smartsuite_addin/ribbon.xml` 的内容粘贴到 Custom UI Editor 中
5. 保存，关闭 Custom UI Editor
6. 在 Excel 中：**文件 → 选项 → 加载项 → 管理 "Excel 加载项" → 转到 → 浏览** → 选择 `SmartSuite_Addin.xlam`
7. 重启 Excel —— 你应该在功能区看到 **"工艺分析"** 标签页

> **说明：** 自定义功能区需要 xlwings Pro（付费版）才能完全自动化。免费版需要借助 Custom UI Editor 工具一次性配置。配置一次后即可永久使用。

#### 方式 C：只用命令行和 Python（无需 Excel 集成）

如果你暂时不需要 Excel 功能区，可以完全通过命令行使用所有分析功能：
...
# 用模板运行分析
smartsuite run templates/example_anova.yaml -i your_data.xlsx
