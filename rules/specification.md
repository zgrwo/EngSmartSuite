...
**EngSmartSuite**（工艺数据分析工具箱）是一个将 Python 统计分析能力与 Web UI 交互体验深度整合的工具，提供覆盖要因分析、DOE/优化、SPC 控制图、过程能力、异常检测、可靠性/MSA、探索性分析等 7 大领域的统计分析方法。

### 核心价值

- 覆盖从要因分析到可靠性工程的工艺统计分析方法（方法总数见 `rules/api-reference.md`）
...
- **TASK_REGISTRY 路由**：全部任务统一注册（数量唯一源为 TASK_REGISTRY），Web/CLI 共享
...
| 统计库 | scipy + statsmodels | 覆盖全部分析方法 |
