...
| **参数** | `show_stats=true`(默认) | 坐标区下沿以**单张统计表**呈现各分组 n/均值/标准差/最大值/最小值（列=分组、与箱体对齐；箱体压缩、图形尺寸不变）；自动附 ANOVA/Kruskal-Wallis 检验结果 |
...
*分组箱线图 — 按原料类型(5组)的不良率分布，散点叠加展示个体差异。各组箱体高度相近，中位数接近。箱体下方为单张统计表：列为分组（与箱体一一对齐），行为 n/均值/标准差/最大值/最小值。*
...
| **参数** | `show_stats=true`(默认) | 次分类生成 3 张分面箱线图（一/二/三车间各一张）；每张分面下方各带该面板的统计表 |
...
- **统计表**：`show_stats`（默认 `true`）在坐标区下沿绘制**单张统计表**——列为分组（与箱体 x 位置一一对齐），行为 `n` / 均值 / 标准差 / 最大值 / 最小值，数值口径与 `group_statistics` 表逐位一致；开启时 `n` 不再重复出现在 X 轴刻度标签上（关闭则回到 `类别\n(n=…)` 旧样式）。表格占用坐标区下沿固定比例，箱体自动压缩，**图形整体尺寸不变**。次分类分面模式每张面板各带一张表（按面板内各组重算，与箱体一一对应）。
...
| 消息 / 表现 | 原因 | 正确做法 |
| :--- | :--- | :--- |
| `规格限无效: USL (10.0) ≤ LSL (100.0)，请确保 USL > LSL。` | USL 与 LSL 填反 | USL 填上限、LSL 填下限 |
| `判定: 未提供规格限` | 只给了测量值、没给规格限 | Cp/Cpk 必须提供 USL/LSL（只给一侧也行，按单侧计算） |
| `检测到 40 个单值点（n=1），已在 X-bar 图中显示为方块标记，不参与极差/标准差计算。` | 未选子组列或子组列名拼错 | 核对子组列；确认数据是「每个子组多个样本」的长表格式 |
| `有效数据不足(至少20个点)` | 变点检测样本太少 | 至少 20 个观测 |
| 控制限位置与预期不符 | 控制限由**本批数据**计算，不是历史基线 | 需固定控制限时用 `ucl`/`lcl`/`cl` 参数显式指定 |

> 颜色约定：控制限为**金黄虚线**、规格限为**红色实线**、目标值灰点线。图上出现红色实线即代表规格限，可据此快速区分两类限。
