...
| **参数** | `show_stats=true`(默认) | 每个箱体正下方标注 n/均值/标准差/最大值/最小值（箱体压缩、图形尺寸不变）；自动附 ANOVA/Kruskal-Wallis 检验结果 |
...
*分组箱线图 — 按原料类型(5组)的不良率分布，散点叠加展示个体差异。各组箱体高度相近，中位数接近。每个箱体正下方独立标注该组的 n/均值/标准差/最大值/最小值。*
...
| **参数** | `show_stats=true`(默认) | 次分类生成 3 张分面箱线图（一/二/三车间各一张）；分面模式不加统计标注 |
...
- **统计标注**：`show_stats`（默认 `true`）在每个箱体正下方标注该组 `n` / 均值 / 标准差 / 最大值 / 最小值，数值口径与 `group_statistics` 表逐位一致；开启时 `n` 不再重复出现在 X 轴刻度标签上（关闭则回到 `类别\n(n=…)` 旧样式）。标注占用坐标区下沿约 1/4，箱体自动压缩，**图形整体尺寸不变**。次分类分面模式（每张面板面积较小）不加标注。### 7.11 非参数控制图 (`spc_nonparametric`) 🆕
