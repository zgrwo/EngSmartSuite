...
| 消息 / 表现 | 原因 | 正确做法 |
| :--- | :--- | :--- |
| `需要 2 个评定者列` | 一致性分析只选了 1 列 | 在「因子（X）」中选**恰好 2 列**（两名评定者的评分） |
| `至少需要 2 个题项列` | Cronbach α 只选了 1 个题项 | 选 ≥2 个同维度题项列 |
| `正态性评估: 1/1 列满足正态性` | 小样本下该判定很不稳定（n 很小也可能给结论） | n < 20 时不要把正态性检验当判决依据，改看 Q-Q 图与偏度 |
| 功效分析未报错但结果不符合预期 | 参数留空会被**自动填默认值**（不会报错） | 显式选择检验类型与模式，不要依赖默认 |

> 反向计分题需先自行反转（如 `最大值 + 1 - 原值`）再传入 —— 工具不会自动识别反向题。
