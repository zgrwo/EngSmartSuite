...
- **反演请求行**：在面板下方逐行录入来料/固定条件与各输出目标值（可调参数留空，由反解计算；固定列留空按历史中位数）。也可沿用"在数据文件中留空可调参数列"的方式。
...
- **方程表决定可换算性**：`model_equations` 中线性/poly/rate 给出原始单位方程与解析反解，可直接手算核对；`gpr`/`gbm` 为核方法/树集成，标注"无解析表达式"，只能按推荐值执行。
