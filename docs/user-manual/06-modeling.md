...
| 消息 / 表现 | 原因 | 正确做法 |
| :--- | :--- | :--- |
| `有效样本量(4)不足，需要至少5条` | 有效样本少于「因子数 + 1」（缺失值 dropna 后更少） | 减少因子个数或补充数据 |
| `目标列需要恰好 2 个不同值` | Logistic / ROC 的目标列不是二值 | 先把目标编码为 0/1 |
| `需要至少 1 个因子列` | 决策树未选 X | 至少选 1 个因子 |
| `需要提供因子定义 (factors)，格式: [{'name': ..., 'levels': [...]}]` | DOE 设计只选了列名、未填因子水平 | 在参数面板填写每个因子的水平值 |
| `需要提供优化目标 (objectives)` | 多目标优化未指定目标 | 指定要优化的输出列与优化方向 |
| `需要提供参数搜索范围 (ranges)` | 网格搜索未给范围 | 为每个可调参数填写搜索区间 |
| `未识别到输出列（前缀 output/输出）` | 反解未按前缀约定命名输出列 | 列名加 `Output*`/`输出*` 前缀，或显式指定角色参数 |

> 响应面/回归的**调整 R² 为负值是正常输出**（模型弱于均值预测时），不代表程序出错 —— 它说明当前因子不足以解释目标波动。
