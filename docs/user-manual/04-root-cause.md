...
*热力图：红色=正相关，蓝色=负相关，颜色越深 \|r\| 越大。星号(\*)标记 Bonferroni 校正后的显著性水平（\* p<0.05, \*\* p<0.01, \*\*\* p<0.001）。对角线为各变量与自身的完全正相关（r=+1.00）。*
...
*决策树结构图（max_depth=5，图为完整树的前 3 层）：每层一个分裂条件，叶节点输出预测值。因子中冷却时间在根节点附近出现频率最高，灰色 [...] 表示被截断的子树。*
...
![列联表分组柱状图](images/contingency_1.png)

*分组柱状图：各保养日状态（否/是）下原料类型的占比（组内归一化）。两组分布相近，说明两者无关联。*
...
*Wilson Score(浅蓝) vs Clopper-Pearson(深蓝) 95%CI 对比。红色竖线=点估计 91.1%。*
...
| 消息 / 表现 | 原因 | 正确做法 |
| :--- | :--- | :--- |
| `以下列非数值型，无法计算相关性: ['材料']` | 目标列或因子列选了文本列（相关性/VIF/回归等只接受数值列） | 类别列先编码，或改用支持分组列的 `contingency` / `anova` |
| `至少需要 1 个因子列与目标列进行相关性分析，当前无有效因子列` | 未选 X 列 | 在「因子（X）」中至少选 1 列 |
| `没有可用于 ANOVA 分析的特征列` | 分组列名拼错或全为缺失值（**列不存在时也报这一条**） | 核对「分组列」下拉中的实际列名 |
| `目标列「常数」方差为零（常量列）或有效样本不足，无法计算相关性分析。` | 该列所有取值相同 | 换一个有变化的列 |
| `control_vars 必须是列名列表或逗号分隔字符串` | 偏相关参数填了数字 | 填列名的逗号分隔串（如 `温度,压力`） |

> 偏相关（`control_vars`）解读：控制后偏相关远小于零阶相关 → 原相关来自共同趋势（**削弱**）；
> 反而变大 → 该因子被共变量掩盖（**抑制**）。两种情形在结论里必须区分表述，不能都写成「相关」。
