...
**操作**: Y=`不良率`(Y), X=`熔体温度, 模具温度, 注射压力, 冷却时间`(X), 参数 `max_depth: 3` → 点击 **决策树重要性**

**预期结果 (Web UI, n=1000)**:

| 因子 | 内置重要性(Gini) | 排列重要性 | 排列重要性 std |
|------|-----------------|-----------|---------------|
| 冷却时间 | 0.527 | 0.129 | ±0.012 |
| 熔体温度 | 0.375 | 0.070 | ±0.011 |
| 模具温度 | 0.098 | 0.020 | ±0.009 |
| 注射压力 | 0.000 | 0.000 | ±0.000 |

| 元数据 | 值 |
|--------|-----|
| 关键因子 | 冷却时间 |
| 训练 R² | 0.074 |
| CV R² | -0.018（负值 = 模型无预测力） |
| 样本量 | 1000（NaN 由中位数填充） |
| 图表 | 2 张（特征重要性对比 + 决策树结构图） |

> CV R² < 0 表示模型预测能力不如直接用均值——这在随机数据上是正常的。
> 排列重要性比内置 Gini 更可靠：Gini 偏向高基数变量，排列重要性无此偏差。
...
# → 关键因子: 冷却时间, CV R²=-0.035 (直接Python, n=804, NaN由dropna处理)
# Web UI 路径 (n=1000, NaN中位数填充): CV R²=-0.018
```
