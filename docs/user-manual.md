...
*散点图自动绘制 (X, Y) 散点 + 可选拟合线。OLS 模式显示回归方程 (y = β₀ + β₁x) 和 R²。分组时自动着色 + 图例。*
...
**结论**: 全部 40 个分析方法在 Web UI 和 Python 直接调用下产生一致的结果。
...
#   OK anova                       0.3s  ok
#   ...
# Results: 40/40 responded, 0 failed
```
...
### Q: 错误 "请先上传数据文件"
**原因**: 未上传数据文件或上传失败。
**解决**: 重新上传 .xlsx / .xlsm / .csv 文件，确认文件 < 50MB。CSV 文件如含中文建议使用 UTF-8 编码。
