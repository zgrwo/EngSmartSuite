...
**功能**: 用决策树模型评估每个因子对目标的解释力，输出排列重要性（比内置 Gini 更可靠，不受模型结构偏差影响）。
...
#### 数值结果（Web UI ≡ Python） X-bar 图（上）+ R 图（下），含 ±1σ/±2σ/±3σ 区域着色 + 违规点红色标记。
...
#### 解读说明 5 组均值都在 4.17~4.35 之间，中位数在 3.89~4.08，差异极小。ANOVA p=0.615 不显著。各组的 IQR 约 1.3~1.5 说明组内离散度相近。
