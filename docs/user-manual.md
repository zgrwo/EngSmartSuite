...
> 排列重要性比内置 Gini 更可靠——Gini 偏向高基数变量。综合重要性 = max(排列重要性, 0)。
> Web UI 和 Python 的排序完全一致（冷却时间>熔体温度>模具温度>注射压力），
> 数值差异来自 NaN 处理方式不同（中位数填充 vs 删除）。
...
for _, row in fi.iterrows():
    print(f"{row['因子']}: 排列重要性={row['排列重要性']:.3f}")
# → 冷却时间: 0.261 | 熔体温度: 0.148 | 模具温度: 0.049 | 注射压力: 0.025
# (直接Python, n=804, NaN由dropna处理。趋势与Web UI一致)
```
