...
def process_audit(
    ...
    if missing_pct > 5:
        ...
        health_checks.append({"检查项": "数据完整性", "状态": "✓ 正常", "详情": "缺失率 < 5%"})

    # ── 2. 相关性 ──
    if len(numeric_features) >= 2:
        try:
            req = AnalysisRequest(
                task="correlation", data=df, target_col=target_col, feature_cols=numeric_features
            )
            r = orchestrate(req)
            _close_figures(r)
    ...
    if len(numeric_features) >= 3:
        try:
            req = AnalysisRequest(
                task="vif", data=df, target_col=target_col, feature_cols=numeric_features
            )
            r = orchestrate(req)
            _close_figures(r)
            ...
            health_checks.append(
                {"检查项": "共线性诊断", "状态": "✗ 失败", "详情": f"计算异常 ({type(e).__name__})"}
            )
    ...
    if usl is not None and lsl is not None:
        try:
            req = AnalysisRequest(
                ...
                params={"usl": usl, "lsl": lsl, "target": target},
            )
            r = orchestrate(req)
            _close_figures(r)
            ...
            health_checks.append(
                {"检查项": "过程能力", "状态": "✗ 失败", "详情": f"计算异常 ({type(e).__name__})"}
            )
    ...
    if time_order and len(df) >= 10:
        try:
            req = AnalysisRequest(
                task="trend_forecast", data=df, target_col=target_col, params={"forecast_steps": 5}
            )
            r = orchestrate(req)
            _close_figures(r)
            ...
            health_checks.append(
                {"检查项": "过程稳定性", "状态": "✗ 失败", "详情": f"计算异常 ({type(e).__name__})"}
            )
    ...
    try:
        req = AnalysisRequest(
            ...
            data=df,
            target_col=target_col,
            feature_cols=numeric_features[:3],
        )
        r = orchestrate(req)
        _close_figures(r)
        ...
        health_checks.append(
            {"检查项": "异常值检测", "状态": "✗ 失败", "详情": f"计算异常 ({type(e).__name__})"}
        )
...
def auto_report(
    ...
    if feature_cols is None:
        feature_cols = [
            c for c in df.columns if pd.api.types.is_numeric_dtype(df[c]) and c != target_col
        ]
    ...
    tasks_to_run = ["correlation", "anova", "normality_check", "distribution_summary"]
    if usl is not None and lsl is not None:
    ...
    # 综合审计
    audit = process_audit(df, target_col=target_col, feature_cols=feature_cols, usl=usl, lsl=lsl)
    ...
    req = AnalysisRequest(
        task="regression", data=df, target_col=target_col, feature_cols=feat_for_html
    )
    r = orchestrate(req)
    if r.status != "ok":
        r.summary = (
...
def export_workbook(df, target_col, feature_cols, output_path, tasks=None):
    ...
    for task in tasks:
        try:
            req = AnalysisRequest(
                task=task, data=df, target_col=target_col, feature_cols=feature_cols
            )
            r = orchestrate(req)
            _close_figures(r)
            if r.status != "ok":
            ...
            for name, table in r.tables.items():
                ...
                for ci, col in enumerate(table.columns):
                    ws.cell(row=row, column=ci + 1, value=str(col)).font = openpyxl.styles.Font(
                        bold=True, color="FFFFFF"
                    )
                    ws.cell(row=row, column=ci + 1).fill = openpyxl.styles.PatternFill(
                        "solid", fgColor=_to_argb(PALETTE["data"]["primary"])
                    )
                row += 1
                # Data
